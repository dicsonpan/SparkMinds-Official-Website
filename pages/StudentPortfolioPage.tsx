...
export const StudentPortfolioPage: React.FC = () => {
  ...
  const handleTranslate = async () => {
    ...
    try {
      ...
      Ensure that all value fields (title, content, descriptions, bio, skills names) are translated.
                     ...
                     {urls.map((url, i) => {
                        ...
                        return (
                            <div key={i} className="relative aspect-video rounded-lg overflow-hidden border border-slate-700/50 shadow-sm bg-slate-900/50">
                                <img src={url} alt="Evidence" loading="lazy" className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
                            </div>
                        );
                     })}
                  </div>
